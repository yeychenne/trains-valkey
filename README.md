...
| **E5 t1-multi-victim** (5-node ring, 2 sequential kills) | 5-node ring; SIGKILL node 1 at T+15s, SIGKILL node 2 at T+45s; remaining 3 must converge | **`missing_keys = []` on every survivor; `DBSIZE = 2 000` on every survivor**. The chaos client acked 1 999 / 2 000 — the 2 000th write completed on the ring and was applied to all three survivor engines, but the client abandoned the ack after 5 s (the `--abandon-secs` honest cap). Stronger than "zero acked loss": the data plane delivered everything, the harness gave up on one ack. Closes the "credibility on small clusters" gap. [REPORT](bench/results/ec2-2026-06-25-e5-matrix/REPORT.md) |
